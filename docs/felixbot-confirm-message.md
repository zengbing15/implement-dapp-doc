...
[Moleculec-es](https://github.com/nervosnetwork/moleculec-es) is a ECMAScript plugin for the molecule serialization system. Use [molecule-es](https://github.com/nervosnetwork/moleculec-es) to compile the schema and generate the javascript file to use.You can find the compiled files [here](https://github.com/zengbing15/felix/tree/master/schema).
The finalized file `UnsignedTransaction.umd.js` includes molecule serialization implementation and molecule deserialization implementation.
...
Use `UnsignedTransaction` function in `UnsignedTransaction.umd.js` to generate serialized ArrayBuffer data,
then use [Reader class](https://github.com/nervosnetwork/ckb-js-toolkit#reader) in CKB-JS-Toolkit to convert the result's format.
...
which displayed how to use the molecule deserialization implementation. Use the tool to deserialize `UnsignedTx.json`, restore the previous transfer transaction, generate the `txSkeleton` and finally use `common.prepareSigningEntries(txSkeleton)` to generate message.
...
```javascript title="/generate-message-tool/schema/UnsignedTransaction.umd.js"
  ...
  ......
```
Then call the corresponding `getXXX()` method to deserialize the result, by the way, use [Reader class](https://github.com/nervosnetwork/ckb-js-toolkit#reader) to convert format.
...
Well, this is a important principle for development DApps on CKB layer1:
